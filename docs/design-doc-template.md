...
- 小循环（秒级动作反馈）：{例：点金堆 → 金币飞行入账}
- 大循环（playable 全程弧线）：{战斗压力型：收集→转化→战斗验证→胜利；解压经营型：小循环 xN → 解锁/扩产 → 繁荣展示}
...
- **操作归属**：{主角移动必须由玩家驱动（点击/摇杆），停手即停在原地；不受玩家掌控的推进不允许（禁止自动结算、自动走位）；自动化必须是玩家购买获得的（雇员/机器）；范围内自动交互（走近自动采集/攻击）属于动作自动化，允许}
- **目标链**：{按顺序列出每个目标；每个小循环闭合时下一个目标必须已在场上，不让玩家重复空转。例：免费雇工 → 20 金雇工 → 40 金开二号矿区 → 60 金开金库(结算)}
- **长期锚点**：{开局第一帧就可见的远期目标（锁定大建筑/区域）及其位置}
- **地图尺度**：{角色高度占画面比例（如 4%）→ 地图分几层、各层放什么}
