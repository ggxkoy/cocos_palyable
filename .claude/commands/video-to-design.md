...
3. **逐帧分析**（用 Read 工具看图）
   - **先找即时爽感兑现点**：买量试玩的核心是“操作后马上变强/变多/爆开/推进”。对每个领取、购买、合成、解锁和击杀，分别记录同帧 UI/数值变化、100ms 内逻辑变化、300ms 内动画/音效/战力兑现；禁止设计为下一波、下一状态或回到指定位置后才生效。
   ...
   - **非线性检查**：时间轴只是"参考录像里发生的顺序"，不是脚本——落地时一切事件必须由玩家行为/世界状态触发（购买生效、敌潮清场、站上目标牌），禁止"第 N 秒触发 X"的秒表事件；多个目标牌应同时在场，顺序靠价格大小关系引导。
...
5. **输出策划案**
   ...
   - **「模板映射」表必须机器可校验**：`配置项` 一列写成目标 Config 的点路径并用反引号
     ...
     填不出来的字段保留模板默认值并注明。
   - 关键帧截图挑 8-15 张有代表性的复制到 `docs/design/<视频名称>-frames/`，覆盖开局、首次引导、核心循环、首次解锁、中段压力/扩产、高潮、结算。
   - 必须在正文对应表格或章节中使用 Markdown 图片语法直接嵌入关键帧，不能只写文件名。最终文档应接近参考 PDF 的图文密度，而不是纯文字摘要。
   - 必须单列“即时爽感链”及验收时延：输入/碰撞后同帧 UI 变化、100ms 内逻辑生效、300ms 内完成主要视听和玩法反馈；关键奖励不得延迟结算。
   - 如果视频无法读取或无法抽出关键帧，不要生成“证据受限版”占位策划案；应让任务失败并明确提示用户改为上传本地视频。
