...
| 行为树经营 | `crew` | 主角点击驱动+范围自动采集，雇员自动化，目标链到亲手开金库 | 行为树循环 + 全购买流通关 |
| **3D 模块拼装** | `salvage3d` | 等距 3D：打捞残骸→回收→目标链→敌潮防御演出；由 11 个可复用模块拼成 | 纯逻辑组装冒烟全绿（链条/敌潮/弹药） |

> **3D 模块层是新 playable 的推荐形态**（参考视频均为等距 3D）：每个功能 = Sim（纯逻辑）+ Module（视觉），
> 通过事件总线拼装；新 playable 只需一份 Config + 模块清单。详见 `docs/workflow.md` 的「3D 模块层」。
